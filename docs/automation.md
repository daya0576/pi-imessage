...
Follow-up implementation: jobs may opt in with `dailySummary: true`. At/after 21:00 Asia/Shanghai the service queues one persisted daily digest per chat/day, including paused tasks; restart checks do not requeue the same digest. The production notifier verifies an exact outgoing Messages row (text or typedstream attributedBody) with is_sent=1/error=0 before outbox acceptance. Stable notice IDs allow late-success lookup before retries. This remains local send verification, not delivery/read confirmation. General server authentication and remote heartbeat remain outside this MVP.
